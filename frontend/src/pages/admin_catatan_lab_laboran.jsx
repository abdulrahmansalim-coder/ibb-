...
import { getLabLogbooks, getLabLogbookById, updateLabReview } from '../api/adminService';
...
function Sidebar() {
  ...
  const handleCatatanClick = () => {
    alert('📋 Anda berada di halaman Catatan');
  };
...
// --- 3. MAIN COMPONENT ---
export default function AdminCatatanLabLaboran() {
  const navigate = useNavigate();
  ...
  // Load all logbooks for selection dropdown if needed
  useEffect(() => {
    fetchLogbooks();
  }, []);
  ...
  const fetchLogbooks = async () => {
    try {
      ...
      console.error('Error load lab logbooks:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadLogbookDetail = (id, list = logbooks) => {
    ...
    if (found) {
      ...
      setCatatan(found.catatan_laboran || '');
    }
  };
  ...
  const handleSaveAndInput = async () => {
    ...
    try {
      ...
      if (res.success) {
        alert('✅ Catatan Laboran berhasil disimpan ke database!');
        navigate('/admin/lab-logbook-laboran');
      } else {
        alert('❌ Gagal menyimpan: ' + res.message);
      }
    } catch (err) {
      console.error('Error saving lab catatan laboran:', err);
      ...
      <div className="w-full max-w-[1200px] bg-white rounded-lg shadow-sm flex flex-col overflow-hidden border border-gray-200">
        ...
        <div className="flex flex-1">
          ...
          <main className="flex-1 bg-[#F3F4F6] p-8 flex flex-col">
            {/* Top Info & Logbook Selector */}
            <div className="flex justify-between items-start mb-6">
              <div className="max-w-2xl">
                <h2 className="font-serif font-bold text-lg text-ibb-dark mb-2">Catatan Laboran Untuk Researcher (Lab)</h2>
                <p className="text-sm text-gray-600 leading-relaxed">
                  Berikan instruksi operasional, evaluasi teknis, atau catatan keselamatan khusus untuk praktikum hari ini. Seluruh catatan akan tersimpan secara terstruktur di database.
                </p>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex-1 flex flex-col">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xs font-bold text-gray-500 tracking-wider">ISI CATATAN DARI LABORAN</h3>
              ...
              <textarea 
                ...
                placeholder="Tuliskan catatan teknis atau instruksi khusus untuk Researcher di sini..."
                value={catatan}

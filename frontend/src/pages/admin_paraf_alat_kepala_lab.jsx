...
function Sidebar() {
  ...
  const handleParafDigitalClick = () => {
    alert('📝 Anda berada di halaman Paraf Digital Kepala Lab (Alat)');
  };
...
// --- 3. MAIN COMPONENT ---
export default function AdminParafAlatKepalaLab() {
  const navigate = useNavigate();
  const location = useLocation();

  const user = JSON.parse(localStorage.getItem('user') || '{}');
  const [namaLaboran, setNamaLaboran] = useState(user.nama_lengkap || user.full_name || 'Kepala Laboratorium ABEL');
  ...
  const fetchLogbooks = async () => {
    try {
      ...
      console.error('Error load alat logbooks for paraf kepala lab:', err);
    }
  };
  ...
  const loadLogbookDetail = (id, list = logbooks) => {
    ...
    if (found) {
      ...
      if (found.paraf_kepala_lab) {
        setPreviewUrl(found.paraf_kepala_lab);
        setIsVerified(true);
      }
      if (found.status_paraf_kepala_lab === 'rejected' || found.status_paraf_kepala_lab === 'approved') {
        setStatus(found.status_paraf_kepala_lab);
      } else {
        setStatus('approved');
      }
    }
  ...
  const handleSimpanPersetujuan = async () => {
    ...
    try {
      ...
      if (res.success) {
        alert('✅ Paraf Digital Kepala Lab (Alat) berhasil disimpan ke database!');
        navigate('/admin/alat-logbook-kepala-lab');
      } else {
        alert('❌ Gagal menyimpan: ' + res.message);
      }
    } catch (err) {
      console.error('Error saving alat paraf kepala lab:', err);
        ...
        <div className="flex flex-1">
          ...
          <main className="flex-1 bg-[#F3F4F6] p-8 flex flex-col">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
              <div>
                <h2 className="font-serif text-2xl font-bold text-[#0B1C33] mb-2">Validasi &amp; Paraf Digital Kepala Lab (Alat)</h2>
                <p className="text-sm text-gray-600 max-w-xl">
                  Unggah paraf digital resmi Kepala Laboratorium untuk persetujuan akhir logbook penggunaan alat/instrumen laboratorium di database.
                </p>
              </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-8 mb-8 relative">
              <div className="flex flex-col items-center mb-6">
                <h3 className="font-serif text-lg font-bold text-[#0B1C33] mb-1">Area Paraf Digital Kepala Lab (Alat)</h3>
              </div>
            ...
            <div className="bg-[#0B1C33] text-white rounded-lg p-6 mb-8">
              <div className="flex items-center gap-2 mb-3">
                <h4 className="font-semibold text-sm text-[#FCD385]">PENGESAHAN KEPALA LABORATORIUM</h4>
              </div>
              <ul className="text-xs space-y-1.5 list-disc list-inside text-gray-300 ml-1">
                <li>Paraf Kepala Lab merupakan validasi tingkat institusional untuk penggunaan alat riset.</li>
                <li>Tanda tangan digital tersimpan otomatis ke database MySQL &amp; audit trail.</li>
                <li>Status paraf logbook alat akan disimpan sesuai pilihan (Approved / Rejected).</li>
              </ul>
            </div>

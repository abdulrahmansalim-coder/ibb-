...
// --- 6. COMPONENT TABLE ROW (Kepala Lab Version) ---
...
export default function AdminLabLogbookKepalaLab() {
  ...
  const fetchDbData = async () => {
    try {
      ...
      console.error('Error load lab logbooks for kepala lab from DB:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadPDF = () => {
    ...
    const title = selectedLab === 'all' 
      ? "General LogBook Laboratorium - All Rooms (Kepala Lab View)" 
      : `General LogBook Laboratorium - Sub-Lab ${selectedLab} (Kepala Lab View)`;
    ...
    const fileName = selectedLab === 'all' 
      ? "General_LogBook_Laboratorium_All_Rooms_KepalaLab.pdf" 
      : `General_LogBook_Laboratorium_SubLab_${selectedLab}_KepalaLab.pdf`;
    doc.save(fileName);
  };
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
                <div>
                  <h2 className="text-xl sm:text-2xl font-serif font-bold text-[#1A233A]">
                    General Data LogBook Laboratorium (Kepala Lab)
                  </h2>

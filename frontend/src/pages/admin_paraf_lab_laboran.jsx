...
function Sidebar() {
  ...
  const handleParafDigitalClick = () => {
    alert('📝 Anda berada di halaman Paraf Digital Laboran');
  };
...
// --- 3. MAIN COMPONENT ---
export default function AdminParafLabLaboran() {
  const navigate = useNavigate();
  ...
  const fetchLogbooks = async () => {
    try {
      ...
      console.error('Error load lab logbooks for paraf:', err);
    }
  };
  ...
  const handleSimpanPersetujuan = async () => {
    ...
    try {
      ...
      const payload = {
        role: 'laboran',
        paraf: previewUrl, // Data URL string base64
      ...
      if (res.success) {
        alert('✅ Paraf Digital Laboran berhasil disimpan ke database!');
        navigate('/admin/lab-logbook-laboran');
      } else {
        alert('❌ Gagal menyimpan: ' + res.message);
      }
    } catch (err) {
      console.error('Error saving lab paraf laboran:', err);
        ...
        <div className="flex flex-1">
          ...
          <main className="flex-1 bg-[#F3F4F6] p-8 flex flex-col">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
              <div>
                <h2 className="font-serif text-2xl font-bold text-[#0B1C33] mb-2">Validasi &amp; Paraf Digital Laboran (Lab)</h2>
                <p className="text-sm text-gray-600 max-w-xl">
                  Unggah atau simpan tanda tangan digital resmi laboran untuk memvalidasi logbook aktivitas laboratorium secara permanen.
                </p>
              </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-8 mb-8 relative">
              <div className="flex flex-col items-center mb-6">
                <h3 className="font-serif text-lg font-bold text-[#0B1C33] mb-1">Area Paraf Digital Laboran (Lab)</h3>
              </div>
            ...
            <div className="bg-[#0B1C33] text-white rounded-lg p-6 mb-8">
              <div className="flex items-center gap-2 mb-3">
                <h4 className="font-semibold text-sm text-[#FCD385]">SOP LABORATORIUM</h4>
              </div>
              <ul className="text-xs space-y-1.5 list-disc list-inside text-gray-300 ml-1">
                <li>Pastikan data log book mahasiswa telah sesuai dengan SOP Lab.</li>

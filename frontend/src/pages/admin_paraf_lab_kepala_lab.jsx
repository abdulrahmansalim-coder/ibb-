...
function Sidebar() {
  ...
  const handleCatatanClick = () => {
    navigate('/admin/catatan-lab-kepala-lab');
  };

  const handleParafDigitalClick = () => {
    alert('📝 Anda berada di halaman Paraf Digital Kepala Lab');
  };
...
// --- 3. MAIN COMPONENT ---
export default function AdminParafLabKepalaLab() {
  const navigate = useNavigate();
  ...
  const fetchLogbooks = async () => {
    try {
      ...
      console.error('Error load lab logbooks for klab paraf:', err);
    }
  };
  ...
  const handleSimpanPersetujuan = async () => {
    if (!selectedLogbookId) {
      alert('⚠️ Pilih data logbook lab terlebih dahulu!');
      return;
    }
    if (!previewUrl) {
    ...
    try {
      ...
      if (res.success) {
        alert('✅ Paraf Digital Kepala Lab berhasil disimpan ke database!');
        navigate('/admin/lab-logbook-kepala-lab');
      } else {
        alert('❌ Gagal menyimpan: ' + res.message);
      }
    } catch (err) {
      console.error('Error saving lab paraf kepala lab:', err);
        ...
        <div className="flex flex-1">
          ...
          <main className="flex-1 bg-[#F3F4F6] p-8 flex flex-col">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
              <div>
                <h2 className="font-serif text-2xl font-bold text-[#0B1C33] mb-2">Validasi &amp; Paraf Digital Kepala Lab (Lab)</h2>
                <p className="text-sm text-gray-600 max-w-xl">
                  Unggah tanda tangan digital Kepala Laboratorium untuk pengesahan resmi logbook penggunaan ruang laboratorium di database.
                </p>
              </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-8 mb-8 relative">
              <div className="flex flex-col items-center mb-6">
                <h3 className="font-serif text-lg font-bold text-[#0B1C33] mb-1">Area Paraf Digital Kepala Lab (Lab)</h3>
              </div>
            ...
            <div className="bg-[#0B1C33] text-white rounded-lg p-6 mb-8">
              ...
              <ul className="text-xs space-y-1.5 list-disc list-inside text-gray-300 ml-1">
                <li>Paraf Kepala Lab merupakan validasi tingkat institusional untuk pelaporan kegiatan riset.</li>

...
function Sidebar() {
  ...
  const handleParafDigitalClick = () => {
    navigate('/admin/paraf-alat-laboran');
  };
...
// --- 3. MAIN COMPONENT ---
export default function AdminCatatanAlatLaboran() {
  const navigate = useNavigate();
  ...
  const fetchLogbooks = async () => {
    try {
      ...
      console.error('Error load alat logbooks for laboran:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveAndInput = async () => {
    ...
    try {
      ...
      if (res.success) {
        alert('✅ Catatan Laboran (Alat) berhasil disimpan ke database!');
        navigate('/admin/alat-logbook-laboran');
      } else {
        alert('❌ Gagal menyimpan: ' + res.message);
      }
    } catch (err) {
      console.error('Error saving alat catatan laboran:', err);
          ...
          <main className="flex-1 bg-[#F3F4F6] p-8 flex flex-col">
            <div className="flex justify-between items-start mb-6">
              <div className="max-w-2xl">
                <h2 className="font-serif font-bold text-lg text-[#0B1C33] mb-2">Catatan Laboran Untuk Penggunaan Alat</h2>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex-1 flex flex-col">
              ...
              <textarea 
                ...
                placeholder="Tuliskan catatan teknis atau instruksi khusus laboran untuk penggunaan alat di sini..."
                value={catatan}

    ...
    <aside className="hidden lg:flex w-56 xl:w-64 bg-[#F8F9FA] border-r border-[#E2E8F0] flex flex-col justify-between py-8 px-4 flex-shrink-0 min-h-[calc(100vh-80px)]">
      <div>
        ...
        <nav className="space-y-1">
          <Link to="/admin/alat-logbook-laboran" className="flex items-center px-4 py-2.5 rounded-lg font-medium transition bg-[#1a2332] text-white">
            <span className="text-sm">Data LogBook Alat</span>
          </Link>
          <Link to="/admin/lab-logbook-laboran" className="flex items-center px-4 py-2.5 text-slate-600 hover:bg-slate-100 rounded-lg font-medium transition">
            <span className="text-sm">Data LogBook Lab</span>
          </Link>
        </nav>
      </div>
...
// --- 5. COMPONENT DIRECT APPROVE / REJECT LABORAN ---
function LaboranActionButtons({ row, onStatusUpdate, updatingId }) {
  const isUpdating = updatingId === row.id;
  const currentStatus = row.parafLaboran?.toLowerCase() || 'pending';

  return (
    <div className="flex flex-col items-center gap-1.5 min-w-[150px]">
      <SignatureStatus status={currentStatus} />
        ...
        <button
          ...
          className={`px-2.5 py-1 rounded text-[11px] font-bold transition-all flex items-center gap-1 shadow-sm ${
            currentStatus === 'approved'
              ...
              : 'bg-green-50 text-green-700 border border-green-300 hover:bg-green-600 hover:text-white'
          }`}
          title="Approve Penggunaan Alat"
        >
          <span>✓</span>
          <span>Approve</span>
        </button>
...
// --- 6. COMPONENT TABLE ROW (Laboran Version - Alat) ---
function DataRow({ row, onStatusUpdate, updatingId }) {
  return (
    <tr className="hover:bg-gray-50">
      ...
      <td className="px-2 sm:px-4 py-3 sm:py-4 align-top text-center min-w-[160px]">
        <div className="mt-1 sm:mt-2">
          <LaboranActionButtons
            row={row}
...
export default function AdminAlatLogbookLaboran() {
  ...
  const fetchDbData = async () => {
    try {
      ...
      console.error('Error load alat logbooks from DB:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    try {
      ...
      const res = await updateAlatReview(id, {
        role: 'laboran',
        status: newStatus,
      ...
      if (res.success) {
        setTableData(prev => prev.map(item => {
          if (item.id === id) {
            return { ...item, parafLaboran: newStatus };
          }
          return item;
        }));
      } else {
      ...
      console.error('Error update status laboran alat:', err);
      alert('❌ Error update status: ' + (err.response?.data?.message || err.message));
    } finally {
      setUpdatingId(null);
    }
  };
  ...
  const handleDownloadPDF = () => {
    ...
    const title = selectedLab === 'all'
      ? "General LogBook Penggunaan Alat - All Tools (Laboran View)"
      : `General LogBook Penggunaan Alat - Sub-Lab ${selectedLab} (Laboran View)`;
    ...
    const fileName = selectedLab === 'all'
      ? "General_LogBook_Alat_All_Laboran.pdf"
      : `General_LogBook_Alat_SubLab_${selectedLab}_Laboran.pdf`;
    doc.save(fileName);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F1F3F5] p-2 sm:p-3 md:p-4 lg:p-8 font-sans">
      <div className="max-w-7xl mx-auto bg-white shadow-lg rounded-sm overflow-hidden">
          ...
          <main className="flex-1 bg-white p-3 sm:p-4 md:p-6 lg:p-8 min-w-0">
            <div className="max-w-full">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
                <div>
                  <h2 className="text-xl sm:text-2xl font-serif font-bold text-[#1A233A]">
                    General Data LogBook Penggunaan Alat (Laboran)
                  </h2>
                    ...
                    <table className="w-full text-left text-[9px] sm:text-xs whitespace-nowrap">
                      <thead className="bg-[#E9EBED] text-[#6B7280] font-bold uppercase tracking-wider text-[8px] sm:text-[10px]">
                        <tr>
                          ...
                          <th className="px-2 sm:px-4 py-2 sm:py-4 text-center min-w-[160px]">PERSETUJUAN LABORAN</th>
              ...
              <div className="flex justify-end pt-2 sm:pt-4 pb-4 sm:pb-8">
                <Link
                  to="/"
                  className="px-4 sm:px-6 py-2 sm:py-2.5 rounded border border-[#E2E8F0] bg-white text-[#1A233A] hover:bg-gray-50 text-xs sm:text-sm font-semibold transition"
                >
                  Kembali ke Beranda
                </Link>
              </div>
        ...
        <footer className="bg-[#0B1C33] py-4 px-4 sm:px-8 border-t border-slate-700">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-2">
            ...
            <div className="flex items-center gap-4">
              <button
                ...
                className="bg-red-600 hover:bg-red-700 text-white text-xs font-semibold px-4 py-1.5 rounded transition flex items-center gap-1.5 shadow-sm"
              >
                <span>Unduh PDF</span>
              </button>
            </div>
